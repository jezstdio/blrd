...
app.use(
  cors({
    origin: (origin, callback) => {
      ...
      const whitelist = [`https://localhost:${PORT}`, "https://blrd.jez.hu"];
...
app.get("/votes/popular", async (req, res) => {
  ...
  const publicVotes = votes
    .sort((a, b) => {
      const aa =
        a.teams.red.scores[a.teams.red.scores.length - 1] +
        a.teams.blue.scores[a.teams.blue.scores.length - 1];
      const bb =
        b.teams.red.scores[b.teams.red.scores.length - 1] +
        b.teams.blue.scores[b.teams.blue.scores.length - 1];
    ...
    .filter((vote) =>
      category
        ? vote.category.some((cat) => cat.toLowerCase() === category && vote)
        : vote
    ) // filter depending on category
    .filter(
      (vote) => vote.times.end[vote.times.end.length - 1] > new Date().getTime()
    ) // is the vote still going?
    .filter(
      (vote) =>
        vote.times.begin[vote.times.begin.length - 1] < new Date().getTime()
    ) // is the vote begun?
...
app.get("/votes/open", async (req, res) => {
  ...
  const openVotes = votes
    .sort((a, b) => {
      const aa =
        a.times.begin[a.times.begin.length - 1] +
        a.times.begin[a.times.begin.length - 1];
      const bb =
        b.times.begin[b.times.begin.length - 1] +
        b.times.begin[b.times.begin.length - 1];
...
app.get("/votes/ended", async (req, res) => {
  ...
  const endedVotes = votes
    .sort((a, b) => {
      const aa =
        a.times.end[a.times.end.length - 1] +
        a.times.end[a.times.end.length - 1];
      const bb =
        b.times.end[b.times.end.length - 1] +
        b.times.end[b.times.end.length - 1];
...
app.get("/vote", async (req, res) => {
  ...
  const isOpen =
    vote.times.end[vote.times.end.length - 1] > new Date().getTime() &&
    vote.times.begin[vote.times.begin.length - 1] < new Date().getTime()
      ? true
      : false;
...
app.ws("/:vote", async (ws, req) => {
  ...
  ws.on("message", async (msg) => {
    ...
    if (message.type === "vote") {
      ...
      connections[message.vote.id].forEach((connection) =>
        connection.send(JSON.stringify({ type: "vote", vote }))
      );
    }
  });
...
async function cronTesting() {
  ...
  votes.forEach((vote) => {
    if (vote.times.begin[vote.times.begin.length - 1] > new Date().getTime()) {
      const dates = {
        seconds: new Date(
          vote.times.begin[vote.times.begin.length - 1]
        ).getSeconds(),
        minutes: new Date(
          vote.times.begin[vote.times.begin.length - 1]
        ).getMinutes(),
        ...
        month:
          new Date(vote.times.begin[vote.times.begin.length - 1]).getMonth() +
          1,
      };

      jobs[vote._id] = cron.schedule(
        `${dates.seconds} ${dates.minutes} ${dates.hours} ${dates.days} ${dates.month} *`,
        () => {
